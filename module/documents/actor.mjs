...
export class QuintessenceSystemActor extends Actor {
    ...
    passTurn() {
        ...
        // Actually like half of these are wrong
        status.burn = math.round(status.burn * 0.5);
        status.bleed = math.round(status.bleed * 0.5);
        status.sinking = math.round(status.sinking * 0.5);
        status.rupture = math.round(status.rupture * 0.5);
        status.tremor = math.round(status.tremor * 0.5);
    ...
    onHit() {
        const systemData = actorData.system;
        const health = systemData.health;
        const sanity = systemData.sanity;
        const status = systemData.status;

        health.value -= status.rupture;
        sanity.value -= status.sinking;
